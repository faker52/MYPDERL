...
class GeneticAgent:
    ...
    def update_parameters(self, batch, p1, p2, critic, net):
        state_batch, _, _, _, _ = batch

        p1_action = net.forward(p1(state_batch), state_batch)
        p2_action = net.forward(p2(state_batch), state_batch)
        ...
        actor_action = net.forward(self.actor(state_batch), state_batch)

        # Actor Update
        self.actor_optim.zero_grad()
        sq = (actor_action - action_batch)**2
        policy_loss = torch.sum(sq)
...
class Net(nn.Module):
    def __init__(self, args, init=False):
        ...
        # whole Layer 3
        self.w_l3 = nn.Linear(args.state_dim + args.state_dim, l3)
        if self.args.use_ln: self.lnorm3 = LayerNorm(l3)

        # whole Layer 4
        self.w_l4 = nn.Linear(l3, l4)

        if self.args.use_ln: self.lnorm4 = LayerNorm(l4)

        # Out
        self.w_out = nn.Linear(l4, args.action_dim)
...
class DDPG(object):
    ...
    def update_parameters_critic(self, batch, actor):
        ...
        # Critic Update
        next_action_batch = self.net.forward(actor.forward(next_state_batch), next_state_batch)
