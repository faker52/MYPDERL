...
class Agent:
    def __init__(self, args: Parameters, env):
        ...
        self.evolver = utils_ne.SSNE(self.args, self.rl_agent.critic, self.evaluate, self.rl_agent.net)
    ...
    def evaluate(self, agent: ddpg.GeneticAgent or ddpg.DDPG, is_render=False, is_action_noise=False,
        ...
        while not done and times < 1000:
            ...
            action = self.rl_agent.net(torch.FloatTensor(agent.actor.select_action(np.array(state)).reshape(1, -1)).to(self.args.device), torch.FloatTensor(state.reshape(1, -1)).to(self.args.device))
    ...
    def train_ddpg(self, champion):
        bcs_loss, pgs_loss = [], []

        """
        
        
        if len(champion.buffer.memory) > self.args.batch_size * 5:
            '''
            ...
            '''
            print("-----train---ddpg---")
            #batch = self.pop[1].buffer.sample(1)
            #state_batch, action_batch, next_state_batch, reward_batch, done_batch = batch

            #print(self.rl_agent.net(self.pop[1].actor(state_batch), state_batch))
