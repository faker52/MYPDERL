...
class SSNE:
    def __init__(self, args: Parameters, critic, evaluate, net):
        self.current_gen = 0
        self.args = args;
        self.critic = critic
        self.net = net
    ...
    def distilation_crossover(self, gene1: GeneticAgent, gene2: GeneticAgent, net):
        ...
        for epoch in range(12):
            for i in range(iters):
                ...
                losses.append(new_agent.update_parameters(batch, gene1.actor, gene2.actor, copy.deepcopy(self.critic), copy.deepcopy(net)))
    ...
    def epoch(self, pop: List[GeneticAgent], fitness_evals):
        ...
        if self.args.distil:
            ...
            for i, unselected in enumerate(unselects):
                ...
                self.clone(self.distilation_crossover(pop[first], pop[second], copy.deepcopy(self.net)), pop[unselected])
        else:
        ...
        for i in offsprings:
            if random.random() < self.args.crossover_prob:
                ...
                self.clone(self.distilation_crossover(pop[i], pop[off_j], copy.deepcopy(self.net)), pop[i])
